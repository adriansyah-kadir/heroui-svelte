...
{:else if sidebar.isMobile}
	...
	>
		<Sheet.Content
			...
			data-slot="sidebar"
			data-mobile="true"
			class="bg-sidebar max-w-[calc(100svw-50px)] text-sidebar-foreground w-(--sidebar-width) p-0 [&>button]:hidden"
	...
	>
		...
		>
			<div
				...
				class="bg-sidebar group-data-[variant=floating]:border-sidebar-border flex h-full w-full flex-col group-data-[variant=floating]:rounded-[calc(var(--radius-field)+8px)] group-data-[variant=floating]:border group-data-[variant=floating]:shadow-sm"
			>
				{@render children?.()}
			</div>
		</div>
